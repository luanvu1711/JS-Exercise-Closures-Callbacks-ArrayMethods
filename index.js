...
function processSum(numberList, callback/* CODE HERE */) {
  ...
  let getSum = numberList.reduce(function(accum, currentItem){
    return accum + currentItem
  },0)
  return callback(getSum);
}

let callBackNum = function(num){
...
function processDuplicateFree(list, callback/* CODE HERE ONLY AFTER COMPLETING ALL OTHER TASKS */) {
  /* CODE HERE ONLY AFTER COMPLETING ALL OTHER TASKS */
    var uniqueArr = [];
    for (let i = 0; i < list.length; i++){
      if(uniqueArr.indexOf(list[i]) == -1){
        uniqueArr.push(list[i]);
      }
    }
...
*/
function getFullNames(arr/* CODE HERE */) {
  /* CODE HERE */
  let fullNames = [];
...
*/
function firstNamesAllCaps(arr/* CODE HERE */) {
  /* CODE HERE */
  let firstName = [];
  arr.map((element) => {
    firstName.push(element.first_name.toUpperCase())
  })
  return firstName;
}
...
*/
function getRunnersByTShirtSize(arr, tShirtSize/* CODE HERE */) {
  /* CODE HERE */
 return arr.filter((value) => {
   return value.shirt_size == tShirtSize;
 });
...
*/
function tallyUpDonations(arr/* CODE HERE */) {
  /* CODE HERE */
